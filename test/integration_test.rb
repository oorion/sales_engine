...
class IntegrationTest < Minitest::Test
  ...
  def test_it_can_find_the_merchant_with_the_most_revenue
    assert_equal "Dicki-Bednar",
    production_sales_engine.merchant_repository.most_revenue(1).first.name

    assert_equal ["Dicki-Bednar", "Kassulke, O'Hara and Quitzon"],
    production_sales_engine.merchant_repository.most_revenue(2).map { |merchant| merchant.name }
  end

  def test_merchant_can_calculate_revenue
    assert_equal (BigDecimal.new('29973') / 100) * 6,
    sales_engine.merchant_repository.entries.last.revenue
  end

  def test_merchant_can_calculate_revenue_by_date
    assert_equal (BigDecimal.new('29973') / 100) * 6,
    sales_engine.merchant_repository.entries.last.revenue(Date.parse('2012-03-07 12:54:10 UTC'))
  end

  def test_merchant_can_find_favorite_customer
    assert_equal 7,
    production_sales_engine.merchant_repository.entries.first.favorite_customer.id
  end

  def test_merchant_can_find_customers_with_pending_invoices
    assert_equal 3,
    production_sales_engine.merchant_repository.entries.first.customers_with_pending_invoices.length
  end

  def test_merchant_repository_can_find_most_items_sold
    assert_equal 1,
    production_sales_engine.merchant_repository.most_items(1).length

    assert_equal ["Kassulke, O'Hara and Quitzon", "Kozey Group", "Thiel Inc"],
    production_sales_engine.merchant_repository.most_items(3).map { |merchant| merchant.name }
  end

  def test_merchant_repository_can_find_total_revenue_by_date_for_all_merchants
    assert_equal BigDecimal.new('190836805') / 100,
    production_sales_engine.merchant_repository.revenue(Date.parse("2012-03-27 14:54:09 UTC"))
  end
  ...
  def test_create_method_creates_a_new_invoice
    ...
    invoice  = sales_engine.invoice_repository.create(
              { customer: customer,
                merchant: merchant,
                status: "shipped",
  ...
  def test_it_creates_a_new_invoice_item
    ...
    item2         = sales_engine.item_repository.entries[1]
    item3         = sales_engine.item_repository.entries[2]
    invoice_items = sales_engine.invoice_item_repository.create_invoice_item(
                    {items: [item1, item2, item3]})
  ...
  def test_customer_can_get_transactions
    assert_equal 7,
    production_sales_engine.customer_repository.entries.first.transactions.length
  end

  def test_customer_can_get_favorite_merchant
    assert_equal 'Balistreri, Schaefer and Kshlerin',
    production_sales_engine.customer_repository.entries.first.favorite_merchant.name
  end

  def test_item_can_get_best_day
    assert_equal Date.parse('2012-03-10'),
    production_sales_engine.item_repository.entries.first.best_day
  end

  def test_item_repository_can_determine_most_revenue
    assert_equal 2,
    production_sales_engine.item_repository.most_revenue(2).length
  end

  def test_item_repository_can_determine_most_items_sold
    assert_equal 1,
    production_sales_engine.item_repository.most_items(1).length
  end
  ...
  def test_invoice_can_charge
    sales_engine.invoice_repository.entries.first.charge(
      credit_card_number: "4444333322221111",
      credit_card_expiration: "10/13",
      result: "success")
    assert_equal "4444333322221111",
    sales_engine.transaction_repository.entries.last.credit_card_number
  end
end
